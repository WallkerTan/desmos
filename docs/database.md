...
### 3.6. Bảng `follows` (Quan hệ theo dõi một chiều)
...
### 3.7. Bảng `friends` (Quan hệ bạn bè song phương)
...
### 3.8. Bảng `friend_ships` (Yêu cầu kết bạn)
...
### 3.9. Bảng `conversations` (Cuộc hội thoại)
...
### 3.10. Bảng `conversation_members` (Thành viên nhóm chat)
...
### 3.12. Bảng `message_status` (Theo dõi trạng thái Seen tin nhắn)
